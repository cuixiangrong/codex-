...
\begin{itemize}[leftmargin=2em]
    ...
    \item 远程连接测试
    远程连接工具可以使用Windows自带的远程桌面连接（mstsc），输入目标主机的tailscale IP地址（100开头的地址）进行连接
    如果想设置无密码连接，可以设置远程桌面用户的登录选项，允许使用空密码登录（不推荐，安全风险较大）
    secpol.msc - 本地策略 - 安全选项 - 账户：使用空密码的本地账户只允许进行控制台登录，设置为禁用
    \end{verbatim}
\end{itemize}
